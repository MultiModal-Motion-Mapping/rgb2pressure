...
class InsoleModule():
    def __init__(self, basdir=None):
        ...
        self.layernorm = torch.nn.LayerNorm(4, eps=1e-6)
    ...
    def visMaskedPressure(self, data):
        ''' vis pressure infered from pressureNet
            ...
            return
                imgColor
        '''
        # filter_data = np.where(data < 0.3, 0, data) # 增添自MI
        ...
        imgColor = cv2.applyColorMap(img, cv2.COLORMAP_JET)
        # imgColor[img == 0] = [0, 0, 0]
        imgColor[~self.maskImg] = 0
    ...
    def linear_normalize(self, data):
        return (data - np.min(data) + 1e-10) / (np.max(data) - np.min(data) + 1e-10)
    
    def linearNorm(self, insole, pixel_weight, avg=False):
        if avg == False:
            pixel_weight = pixel_weight / self.pixel_num
        insole_norm = insole - pixel_weight
        insole_norm = self.linear_normalize(insole_norm)
        return insole_norm
